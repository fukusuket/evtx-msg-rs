...
fn run_binary(evtx_path: &str) -> Vec<String> {
    // In CI the binary is built by `cargo test --test` runner; use `cargo run` to invoke it.
...
fn assert_some_resolved(lines: &[String]) {
    let resolved = lines.iter().filter(|l| {
...
fn directory_input_processes_multiple_files() {
    // Pass the directory containing system logs; expect records from multiple files.
    ...
    // Verify that more than one distinct file appears in the output.
